...
(async()=>{
 ...
 await page.setContent('<main id="repoGeneralPanel"><div id="repoPackageList">LEGACY PACKAGES</div><div class="repo-filters-row"><select id="pubCategoria"><option value="">Todas</option></select><select id="pubPessoa"><option value="">Todas</option></select><label><input id="pubMine" type="checkbox">Só as minhas</label></div><div id="pubList"></div></main><textarea id="privateInput"></textarea><div id="filePreviewOverlay" style="display:none"><div class="modal"><button id="closePreview">Fechar</button><a id="filePreviewDownload">Baixar</a><div id="filePreviewBody"></div></div></div>');
 ...
 // The general repository is identical on desktop/mobile: no fixed legacy blocks and no expanded cards in the list.
 ...
 assert.equal(await page.locator('#pubList > .repository-list-card').count(),3);
 ...
 // Full controls/history appear only after opening a compact card.
 ...
 assert.equal(await page.locator('#repositoryDeck').count(),1);assert.equal(await page.locator('#pubList .repository-actions button').count(),0);assert.equal(await page.locator('#pubList > .repository-list-card').count(),3);
 // Search, person, category and pending filters feed the same compact list/carousel.
 ...
 // A shared folder with two recipients exposes choice only inside the opened file, never as a giant list card.
 ...
 await page.locator('[data-file-id="f0"] [data-file-action="open"]').click();
 await page.locator('#repositoryStage [data-file-action="return"]').click();
 ...
 await page.locator('#repositoryStage [data-file-action="message"]').click();
 ...
 // On-demand package file keeps its receipt in the detail dialog while the list remains compact.
 ...
 await page.locator('[data-file-id="package:pkg:a"] [data-file-action="open"]').click();
 const before=await page.locator('#repositoryStage li').count();
 await page.locator('#repositoryStage [data-file-action="download"]').click();
 await page.waitForFunction(()=>document.querySelector('#repositoryStage ol')?.textContent.includes('Aluno'));
 assert.equal(await page.locator('#repositoryStage li').count(),before+1);
 assert.equal(await page.locator('#pubList [data-file-id="package:pkg:a"] .repository-history').count(),0);
 assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);assert.deepEqual(errors,[]);
 console.log(`PASS Chromium ${width}px: compact unified list, hidden legacy blocks, filters, detail history/actions, navigation, preview, return/message and no horizontal overflow.`);
 await page.close();
