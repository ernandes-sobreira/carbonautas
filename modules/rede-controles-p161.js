/* Carbonautas P165 · controles unificados e leves da Rede
   - filtros + ajustes no mesmo lugar
   - sem MutationObserver de subtree
   - sem remount contínuo
   - um redesenho por ação real
...
let renderRAF=0,retryTimer=0;

function css(){
  if($('#p161RedeControlesStyle'))return;
  const st=document.createElement('style');st.id='p161RedeControlesStyle';st.textContent=`
...
#p135RedeToolbar.p161-unified .p135-rede-modes{display:flex!important;gap:5px;overflow-x:auto;scrollbar-width:none;flex:1 1 auto;min-width:0}
...
function active(){return document.body?.dataset?.view==='rede'||$('#viewRede')?.classList.contains('on')}
function renderOnce(){
  cancelAnimationFrame(renderRAF);renderRAF=requestAnimationFrame(()=>{if(!active())return;try{(window.renderGraph||renderGraph)?.()}catch(e){console.warn('P165 render',e)}})
}
function syncLegend(mode){const canonical={orientacao:'orientation',projetos:'projects',producao:'production'}[mode]||mode;$$('#p158RedeLegend .p158-legend-item').forEach(el=>el.classList.toggle('on',canonical!=='all'&&el.dataset.mode===canonical))}
function bindModes(bar){
  const modes=$('.p135-rede-modes',bar);if(!modes)return;
  const current=String(window.__p135NetworkMode||'all'),normalized={orientacao:'orientation',projetos:'projects',producao:'production'}[current]||current;
  ...
  modes.querySelectorAll('[data-p135-mode]').forEach(btn=>btn.onclick=e=>{
    e.preventDefault();const mode=btn.dataset.p135Mode||'all';if(String(window.__p135NetworkMode||'all')===mode&&btn.classList.contains('on'))return;
    window.__p135NetworkMode=mode;modes.querySelectorAll('[data-p135-mode]').forEach(x=>x.classList.toggle('on',x===btn));syncLegend(mode);
    const insight=$('#p135RedeInsight',bar),tx={all:'Todas as relações',orientation:'Orientações registradas',projects:'Projetos e linhas compartilhadas',production:'Produção compartilhada'};if(insight)insight.textContent=tx[mode]||tx.all;renderOnce()
  });
  const activeBtn=$(`[data-p135-mode="${normalized}"]`,modes)||$('[data-p135-mode="all"]',modes);modes.querySelectorAll('[data-p135-mode]').forEach(x=>x.classList.toggle('on',x===activeBtn));window.__p135NetworkMode=activeBtn?.dataset.p135Mode||'all';syncLegend(window.__p135NetworkMode)
}
function bindFit(){
  const btn=$('#fitBtn');if(!btn)return;btn.title='Ajustar a rede à tela';btn.setAttribute('aria-label','Ajustar a rede à tela');if(!btn.querySelector('span'))btn.insertAdjacentHTML('beforeend','<span>Ajustar visão</span>');
  btn.onclick=e=>{e.preventDefault();try{if(typeof window.fitView==='function')window.fitView();else if(typeof fitView==='function')fitView()}catch(err){console.warn('P165 fitView',err)}}
}
function ensurePlaceholder(){let ov=$('#p135RedeSettings');if(!ov){ov=document.createElement('div');ov.id='p135RedeSettings';document.body.appendChild(ov)}ov.hidden=true;ov.className='p161-placeholder';ov.innerHTML=''}
function pickBase(view){
  const bars=$$('.p135-rede-toolbar',view);let base=bars.find(b=>b.querySelector('#search'))||bars.find(b=>b.id!=='p135RedeToolbar')||bars[0];if(!base)return null;
  const external=$('#p135RedeToolbar',view),legend=$('#p158RedeLegend');if(external&&external!==base){if(legend)base.appendChild(legend);external.remove()}bars.filter(b=>b!==base&&b!==external).forEach(b=>b.remove());base.id='p135RedeToolbar';base.classList.add('p161-unified');return base
}
function mount(){
  if(!active())return false;const view=$('#viewRede');if(!view)return false;const base=pickBase(view);if(!base)return false;
  const heading=$('.p135-rede-heading',base)||$('.p135-rede-title',base);if(heading){heading.classList.remove('p135-rede-title');heading.classList.add('p135-rede-heading');const s=$('small',heading);if(s)s.textContent='pessoas e relações'}
  let modes=$('.p135-rede-modes',base)||$('.p135-modes',base);if(modes){modes.classList.remove('p135-modes');modes.classList.add('p135-rede-modes')}
  let insight=$('#p135RedeInsight',base)||$('.p135-insight',base);if(insight){insight.classList.remove('p135-insight');insight.classList.add('p135-rede-insight')}
  let row=$('.p161-control-row',base);if(!row){row=document.createElement('div');row.className='p161-control-row';if(modes)modes.insertAdjacentElement('beforebegin',row);if(modes)row.appendChild(modes)}
  let toggle=$('#p161SettingsToggle',base);if(!toggle){toggle=document.createElement('button');toggle.type='button';toggle.id='p161SettingsToggle';toggle.className='p161-settings-toggle';toggle.setAttribute('aria-expanded','false');toggle.innerHTML='⚙ Ajustes';row.appendChild(toggle)}
  let panel=$('#p161RedeSettings',base);if(!panel){panel=document.createElement('div');panel.id='p161RedeSettings';panel.innerHTML='<div class="p161-settings-head"><b>Ajustes e filtros</b><small>toque novamente em Ajustes para fechar</small></div>';base.appendChild(panel)}
  const side=$('#side');if(side&&side.parentElement!==panel){side.classList.remove('open');panel.appendChild(side)}const controls=$('#viewRede .graph-controls')||$('#p135RedeSettings .graph-controls')||$('.graph-controls');if(controls&&controls.parentElement!==panel){controls.style.removeProperty('display');panel.appendChild(controls)}if(controls)controls.style.display='';
  ensurePlaceholder();toggle.onclick=()=>{const open=!panel.classList.contains('open');panel.classList.toggle('open',open);toggle.setAttribute('aria-expanded',String(open))};bindModes(base);bindFit();return true
}
function repair(){clearTimeout(retryTimer);if(mount())return;retryTimer=setTimeout(mount,90)}
function boot(){
  css();if(active())repair();
  const mo=new MutationObserver(ms=>{if(ms.some(m=>m.attributeName==='data-view')&&active())repair()});mo.observe(document.body,{attributes:true,attributeFilter:['data-view']});
  setTimeout(()=>{if(active())repair()},180)
}
