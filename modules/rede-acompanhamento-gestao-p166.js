...
function mountPersonAdmin(){
 ...
 box?.remove();box=document.createElement('div');box.className='p166-person-admin';box.dataset.mid=m.id;box.innerHTML=`<button type="button" data-p166-person="edit">⚙ Editar cadastro</button>${m.protegido?'':`<button type="button" class="danger" data-p166-person="delete">Excluir da rede</button>`}`;card.appendChild(box);
 box.onclick=e=>{const b=e.target.closest('[data-p166-person]');if(!b)return;if(b.dataset.p166Person==='edit'){closeDeck();try{(window.openMember||openMember)?.(m.id)}catch(err){console.warn('P166 editar',err)}}else if(b.dataset.p166Person==='delete'){closeDeck();try{const fn=window.removePersonFromManager||removePersonFromManager;Promise.resolve(fn?.(m.id)).finally(()=>{activeMemberId='';setTimeout(()=>prepareTrack(false),120)})}catch(err){console.warn('P166 excluir',err)}}}
}
