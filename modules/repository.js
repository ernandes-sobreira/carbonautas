...
function cardHTML(p){
 const f=files(),can=f.canExchange(p,actor()),turn=p.repositoryTurnMemberId||p.reviewNextRecipientId||f.nextRecipientId(p,p.memberId),target=f.nextRecipientId(p,app().memberId);
...
function openDeck(id){deckIds=[...document.querySelectorAll('#pubList > [data-file-id]')].map(x=>x.dataset.fileId);deckIndex=deckIds.indexOf(id);if(deckIndex<0){deckIds=[id];deckIndex=0}lastFocus=document.activeElement;showDeck()}
function refresh(){if($('#repositoryDeck')?.open)showDeck()}
async function openMessage(id){
 ...
 const target=files().nextRecipientId(p,a.memberId);if(!target)throw Error('Não encontrei a outra pessoa desta troca.');
...
async function preview(id){
 ...
 await root.openFilePreview(p.url,files().publicationTitle(p));
...
async function openReturn(id){
 ...
 returnId=id;returnVersion=files().currentVersion(p);$('#repositoryReturn form').reset();$('#returnTitle').textContent=files().publicationTitle(p);$('#returnRecipient').textContent='Para '+memberName(files().nextRecipientId(p,a.memberId));$('#repositoryReturn').showModal();
}
async function submitReturn(e){
 ...
 try{const p=await files().getPublication(returnId,true);if(files().currentVersion(p)!==returnVersion)throw Error('Outra versão chegou. Feche e reabra o formulário.');const result=await files().commitUploadAndMessage(returnId,file,message);$('#repositoryReturn').close();if(p.packageId)await root.openRepositoryPackage(p.packageId);root.toast(`Versão ${result.nextVersion} enviada. Agora é a vez de ${memberName(result.targetId)}.`)}catch(err){root.toast(err.message||'Não foi possível enviar.')}finally{busy=false;button.disabled=false;button.textContent='Enviar arquivo + mensagem'}
}
...
function boot(){
 ...
 const list=$('#pubList');if(list){const button=document.createElement('button');button.type='button';button.className='btn';button.textContent='▣ Folhear arquivos';button.onclick=()=>{const id=list.querySelector('[data-file-id]')?.dataset.fileId;if(id)openDeck(id);else root.toast('Nenhum arquivo nestes filtros.')};list.before(button)}
