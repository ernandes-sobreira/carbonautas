/* Carbonautas P179 · publicação do Mural robusta no celular
   ...
   - mostra erro real do Firebase/Storage para diagnóstico
...
async function uploadAttachment(ref,file,a){
 ...
 try{
  ...
  console.error('P179 upload do Mural',e);
  return {ok:false,error:e};
 }
}

async function notifyAndPoints(dest,kind,title,text,refId,paraNome,a){
 try{
 ...
 try{
  if(typeof createMuralNotifications==='function')Promise.resolve(createMuralNotifications(dest,{kind,title:(paraNome?'Para '+paraNome+': ':'')+(title||'Nova publicação'),message:text||title||'Nova publicação no Mural',sourceType:'mural-post',sourceId:refId})).catch(e=>console.warn('P179 notificação',e));
 }catch(_e){}
}
...
async function publish(){
 ...
 const text=$('#postText')?.value.trim()||'',title=$('#postTitle')?.value.trim()||'',url=$('#postUrl')?.value.trim()||'',dest=$('#postDest')?.value||'all',kind=$('#postKind')?.value||'atualizacao',date=$('#p176PostDate')?.value||'',time=$('#p176PostTime')?.value||'',file=$('#postFile')?.files?.[0]||null;
 ...
 const uid=window.auth.currentUser.uid,para=dest==='all'?null:dest,paraNome=para?(member(para)?.nome||''):'';
 busy=true;if(btn){btn.disabled=true;btn.classList.add('p177-saving');btn.textContent='Salvando…'}
 status('Salvando a mensagem…');
 try{
  const payload={authorId:a.id,authorUid:uid,authorName:a.name||'',authorFoto:a.foto||'',kind,title,text,externalUrl:url,fileUrl:'',fileName:'',storagePath:'',para,paraNome,actionDate:date,actionTime:time,ts:f.serverTimestamp()};
...
function boot(){addCss();document.addEventListener('click',intercept,true);document.addEventListener('change',scrollPreview,false)}
