/* Carbonautas P155 · acabamento da Rede no mobile
   - espessura visual das orientacoes acompanha a quantidade de registros
   - detalhe da pessoa ocupa a largura util no celular
   - linha do tempo fica compacta, legivel e sem estourar a tela
   Nao grava nem altera dados do Firebase. */
(function(){
'use strict';
...
function injectCss(){
  ...
  st.textContent=`
/* P155 · detalhe da Rede e linha do tempo */
@media (max-width:920px){
  ...
  #viewRede #p128Timeline li:before{
    left:7px!important;
    top:15px!important;
    width:6px!important;
    height:6px!important;
  }
  #viewRede #p128Timeline li time{
    display:block!important;
    margin:0!important;
    font-size:10.5px!important;
    line-height:1.35!important;
    color:#74888e!important;
    white-space:normal!important;
  }
  #viewRede #p128Timeline .p128-more{
    width:100%!important;
...
function bind(){
  ...
  const detail=$('#viewRede #detail');
  if(detail&&!detail.dataset.p155Observed){
    detail.dataset.p155Observed='1';
    new MutationObserver(()=>requestAnimationFrame(polishTimeline)).observe(detail,{childList:true,subtree:true,attributes:true,attributeFilter:['class']});
  }
  polish();
}

function schedulePolish(){[0,40,120,320,850,1500].forEach(ms=>setTimeout(polish,ms));}

document.addEventListener('click',e=>{
  if(e.target.closest?.('[data-p154-mode="orientation"],[data-p135-mode],#graph g.node,#detail .close'))schedulePolish();
},true);
document.addEventListener('visibilitychange',()=>{if(!document.hidden)schedulePolish()});
window.addEventListener('pageshow',schedulePolish);

if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{bind();schedulePolish()},{once:true});
else{bind();schedulePolish()}
[250,700,1600,3000].forEach(ms=>setTimeout(bind,ms));
})();
