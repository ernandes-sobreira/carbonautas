...
const KEY='monaEnabled', SESSION_KEY='monaHelloShown';
...
function css(){
 ...
 #monaP216.on{opacity:1;transform:translateX(7%) translateY(0)}#monaP216.login{bottom:12px}
...
function sync(){if(!enabled())return hide();const inside=appOpen()&&!authVisible();if(authVisible()){clearTimeout(randomTimer);if(!wasInside)hello()}else if(inside){if(!wasInside)setTimeout(()=>show('Bora? 😊',{force:true,duration:1750}),350);scheduleRandom()}wasInside=inside}
