...
function decorateCheckins(){
  document.querySelectorAll('#dashAttention .p56-task').forEach(card=>{
    ...
    if(detail)detail.innerHTML=`<span style="display:inline-flex;padding:4px 7px;border-radius:999px;background:#fff1dd;color:#9b5a00;font-size:9px;font-weight:900;margin-bottom:5px">👀 AÇÃO DE ${String(first(who)).toUpperCase()}</span><div style="font-size:12px;line-height:1.45;color:#405861;font-weight:650">${who} ainda não registrou este check-in. <b>Você só acompanha e pode lembrar a pessoa.</b></div>`;
    if(actions){actions.innerHTML='';const b=document.createElement('button');b.type='button';b.className='btn p72-checkin-remind';b.dataset.memberId=info.memberId;b.dataset.label=what;b.textContent=`💬 Lembrar ${first(who)}`;actions.appendChild(b)}
  })
}
function annotateSmartButtons(){document.querySelectorAll('#dashAttention .p56-task').forEach(card=>{const id=cardActivityId(card);if(!id)return;card.querySelectorAll('.p70-smart-request,.p69-wait').forEach(b=>b.dataset.activityId=id)})}
